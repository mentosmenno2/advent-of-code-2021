...
class Line
{
	...
	public function get_path(): array
	{
		...
		/**
		 * I would like to give credits to Nimithil.
		 * Because PHP is not made for this.
		 * Or at least, not my PHP oriented web-development brain.
		 *
		 * @see Discord Nimithil#9425
		 */
		if ($this->is_diagonal()) {
			$x_iteration = $this->start_coord->get_x() < $this->end_coord->get_x() ? 1 : -1;
			$y_iteration = $this->start_coord->get_y() < $this->end_coord->get_y() ? 1 : -1;

			$y = $this->start_coord->get_y();
			for ($x = $this->start_coord->get_x(); $x !== $this->end_coord->get_x() + $x_iteration; $x += $x_iteration) {
				$coords[] = new Coordinate($x, $y);
				$y       += $y_iteration;
			}
		}
	...
	protected function is_diagonal(): bool
	{
		return $this->start_coord->get_x() !== $this->end_coord->get_x() && $this->start_coord->get_y() !== $this->end_coord->get_y();
	}
}
