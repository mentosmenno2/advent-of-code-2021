...
class Board
{
	...
	{
		$this->squares_rows = array_map(
			function (array $number_row): array {
				return array_map(
					function (int $number): BoardSquare {
	...
	protected function has_won_horizontal(): bool
	{
		foreach ($this->squares_rows as $square_row) {
			$marked_squares = array_filter(
				$square_row,
				function (BoardSquare $square): bool {
	...
	protected function has_won_vertical(): bool
	{
		...
		for ($i = 0; $i < $squares_in_row; $i++) {
			$marked_squares = array_filter(
				array_column($this->squares_rows, $i),
				function (BoardSquare $square): bool {
					return $square->get_marked();
				}
			);
